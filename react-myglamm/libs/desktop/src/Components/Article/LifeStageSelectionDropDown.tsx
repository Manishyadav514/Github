...
interface PropTypes {
  selectedLifeStage: any;
  setSelectedLifeStage: (lifestageId: any) => void;
}
...
const LifeStageSelectionDropDown = (props: PropTypes) => {
  ...
  return (
    <div className="w-4/12 text-center mx-auto pb-8 ">
      <div className="" role="button" tabIndex={0} onClick={() => setIsLifeStageDropDownOpen(prev => !prev)}>
        {!selectedLifeStage?.name ? (
          <div className="flex justify-between bg-color1 px-6 py-4 text-white font-bold rounded">
          ...
          <div role="button" tabIndex={0} className="flex py-3 px-6 bg-color1 items-center justify-between rounded">
      ...
      {isLifeStageDropDownOpen ? (
        <div className="border absolute bg-white md:w-3/12 w-3/12" style={{ zIndex: "5" }}>
          {LIFESTAGES.map(lifestage => (
            <div
              role="button"
              tabIndex={lifestage.id}
              key={`lifestage_${lifestage.id}`}
              className="flex py-4 px-6 border-b border-gray-700 items-center hover:border hover:border-pink-400"
