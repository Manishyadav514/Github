...
const AuthorHeader = (props: PropTypes) => {
  ...
  return (
    <div className="flex items-center pb-2">
      ...
      <div>
        <Link href="/community/feed" legacyBehavior>
          <a>
            {authorDetails?.user_slug === reviewerDetail?.user_slug && (
              <p className="text-[12px] font-medium text-green-600 ">Medically reviewed by</p>
            )}
            <p className="text-sm font-medium capitalize mb-0">{authorDetails?.display_name}</p>
          </a>
        </Link>
        <p className="text-xs font-medium text-gray-400">Author | {authorDetails?.article_count} Articles</p>
      </div>
      <PrimaryBtn
