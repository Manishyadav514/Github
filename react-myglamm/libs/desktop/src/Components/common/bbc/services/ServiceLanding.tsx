...
import ServiceSection from "../../../common/bbc/services/ServiceSection";
...
const ServiceLandingPage = ({ serviceList, pageNo, pageType, isError, seoData, categoryData }: any) => {
  ...
  typeOfPage === "service" &&
    ...
    breadCrumbList?.push({
      ...
      schemaTo: `${GBC_ENV.NEXT_PUBLIC_DOMAIN_URL}/${locationSlug}/${categoryData?.data?.[0]?.slug}`,
    });
  ...
  const breadcrumbSchema =
    ...
    generateBreadCrumbSchema(itemListElement, {
      ...
      url: `${GBC_ENV.NEXT_PUBLIC_CANONICAL_BASE_URL}/${locationSlug}/${subCategorySlug}`,
    });
  ...
  return (
    <main>
      <Head>
        ...
        {pageNo === 1 ? (
          <link rel="canonical" href={`${GBC_ENV.NEXT_PUBLIC_CANONICAL_BASE_URL}${router.asPath.split("?")[0]}`} />
        ) : (
          <link rel="canonical" href={`${GBC_ENV.NEXT_PUBLIC_CANONICAL_BASE_URL}${router.asPath}`} />
        )}
        {pageNo > 1 && (
          <link
            rel="prev"
            href={`${GBC_ENV.NEXT_PUBLIC_CANONICAL_BASE_URL}${router.asPath.split("?")[0]}${
              pageNo > 2 ? `?page=${pageNo - 1}` : ""
            }`}
          />
        )}
        {pageNo < services.data.count / 10 && (
          <link
            rel="next"
            href={`${GBC_ENV.NEXT_PUBLIC_CANONICAL_BASE_URL}${router.asPath.split("?")[0]}?page=${pageNo + 1}`}
          />
        )}
      </Head>
      {typeOfPage === "category" && (
        <div>
          <Image
            alt="banner-service"
            src=" https://files.babychakra.com/site-images/original/desktop-doctor-consult-image.png"
            width="1500"
            height="300"
            className=""
            layout="responsive"
      ...
      {typeOfPage === "category" && (
        <div>
          <Image
            alt="banner-service"
            src="https://files.babychakra.com/site-images/original/desktop-why-BBC.png"
            width="1145"
            height="270"
            className=""
            layout="responsive"
          ...
          <div className="pt-20">
            <Image
              alt="banner-service"
              src="https://files.babychakra.com/site-images/original/desktop-download-app.png"
              width="1141"
              height="282"
              layout="responsive"
              objectFit="cover"
