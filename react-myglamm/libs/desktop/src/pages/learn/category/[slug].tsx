...
const CategoryScreen = (props: PropTypes) => {
  ...
  const breadcrumbSchema = generateBreadCrumbSchema(itemListElement, {
    ...
    url: `${GBC_ENV.NEXT_PUBLIC_DOMAIN_URL}/learn/${categoryResponse?.slug}`,
  });

  const onChangePageLoadData = async (page = 1) => {
  ...
  const getArticleCard = (data: any, dataIndex: any, name: any) => {
    return (
      <div className={`${data.routerId} category_section py-3 px-3  cursor-pointer `} key={`category_${data.id}`}>
    ...
    <>
      <Head>
        {currentPageNumber > 1 && (
          <link
            rel="prev"
            href={`${GBC_ENV.NEXT_PUBLIC_DOMAIN_URL}${
      ...
      />
      <WebContentWrapper className="px-3.5 py-8 mb-3">
        <div className="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-1 gap-4 px-2.5 mb-1 relative">
            ...
            <div className="lg:col-span-2 relative">
              <div className="relative">
                ...
                <div className="absolute left-[20px] bottom-[20px]">
                  <h1
                    className="mb-2 text-xl sm:text-4xl tracking-wider text-white"
                    dangerouslySetInnerHTML={{ __html: categoryResponse.name }}
                  />
                  <p className="mb-2 text-base tracking-wide text-white opacity-70">{categoryResponse?.count} Articles</p>
                </div>
              </div>
            </div>
          ) : null}
          {latestArticleAPIResponse && currentPageNumber <= 1 ? (
            <div className="lg:col-span-1 clearfix clear-both">
              <h3 className="mb-4 mt-0 text-base font-medium capitalize" dangerouslySetInnerHTML={{ __html: "New article" }} />
              <AdSlots id="div-gpt-ad-1648720977785-0" className="mx-auto md:hidden my-4" adSlotData={LEARN_PAGE_ATF} />
              <div className="w-full sm:w-3/6 md:w-3/6 lg:w-full xl:w-full">
                <ArticleCard
                  bannerImage={
        ...
        <div className="clearfix clear-both grid grid-cols-3">
