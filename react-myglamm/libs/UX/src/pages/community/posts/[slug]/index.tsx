...
const PostDetails = ({ postData, filter, errorCode }: any) => {
  const config = getConfig();
  const seoTitle = `${postData?.posts?.[0]?.text?.substring(0, 65)} - ${
    postData?.posts?.[0]?.topicDetails?.topicName
  } - ${WEBSITE_NAME}`;
  const seoDescription = `${postData?.posts?.[0]?.text?.substring(0, 150)} - ${
    postData?.posts?.[0]?.topicDetails?.topicName
  } - ${WEBSITE_NAME}`;
  useCommunitySession();
  ...
  React.useEffect(() => {
    ...
    const digitalData = {
      common: {
        pageName: `web|community|${topicName}|community post|${postDetails.id}`,
        newPageName: "community post",
