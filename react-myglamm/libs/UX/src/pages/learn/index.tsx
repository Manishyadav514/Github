...
import { LearnPageServerInterface, LifeStageInterface } from "@typesLib/articleUtilTypes";
...
import LifeStageSelectionDropDown from "@libComponents/CommonBBC/LifeStageSelectionDropDown";
...
const ArticleLandingPage = (props: LearnPageServerInterface) => {
  ...
  const getData = async (filter = myFilters) => {
    ...
    if (response?.[0]?.status === "fulfilled") {
      const articlesList = response?.[0]?.value?.data.data || [];
  ...
  useEffect(() => {
    GAdispatchTagManagerEvents("Article Listing Ads Loader", {});
  }, [router.asPath]);
  ...
  return (
    <main className="px-4 lg:px-56 py-12">
      <Head>
        ...
        {pageNo > 1 && (
          <link
            rel="prev"
            href={`${GBC_ENV.NEXT_PUBLIC_CANONICAL_BASE_URL}/learn${getExtraPageParams()}${
