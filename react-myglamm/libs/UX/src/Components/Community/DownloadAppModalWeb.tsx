...
const DownloadAppModalWeb = ({ onCloseModal, activeModal }: any) => {
  React.useEffect(() => {
  ...
  return (
    <div>
      ...
      <div className="fixed left-1/2 -translate-x-1/2 bg-white top-20 w-[300px] z-[70]">
        ...
        <img src={getStaticUrl((modalData as any)[activeModal].imgSrc)} alt="" />
        <p className="mt-4 mb-2 w-[70%] display-block mx-auto text-base font-semibold text-center text-color1">
          {(modalData as any)[activeModal].title}
        </p>
        <p className="font-normal text-sm text-center w-[70%] display-block mx-auto">
          {(modalData as any)[activeModal].subtitle}
        </p>
        <div className="flex items-center justify-center my-2">
          <Link href={GOOGLE_PLAY} passHref target="_blank" rel="noopener noreferrer">
            <img src={getStaticUrl("/images/bbc-g3/download-app/app-store1.png")} alt="app store icon" className="mr-2" />
          </Link>
          <Link href={APP_STORE} passHref target="_blank" rel="noopener noreferrer">
...
DownloadAppModalWeb.defaultProps = {
  customClassName: "",
};

export default React.memo(DownloadAppModalWeb);
