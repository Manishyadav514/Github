...
function PDPRecentlyViewedCarousel({ icid, productId}: any) {
  ...
  return (
    <section className={clsx("RecentlyViewProduct mt-2 pt-3")}>
      {recentViewProducts.length > 0 && (
        <>
          <PDPLabel label={t("recentlyViewedProdeucts")} />
        </>
      )}
      <ul
      ...
      >
        {recentViewProducts?.map((product: any, index: number) => (
          <li
            key={product.id}
            className={clsx("mr-2 h-full rounded-3 overflow-hidden mb-2 px-2 py-3")}
            style={{
              width: "150px",
              minWidth: "150px",
            ...
            <div>
              ...
              <div className="mt-1">
                {product.productMeta.isPreOrder ? (
                  ...
                  <CTAButton
                    loading={loading && productPosition === index + 1}
                    CTA={product.shades.length > 1 ? `Show ${t("shade")}` : t("addToBag")}
                    clickAction={() => {
                      miniPDPProductData(product, index + 1);
                    }}
                  />
...
export default React.memo(PDPRecentlyViewedCarousel);
