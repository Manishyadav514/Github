    ...
    <div className="mb-5 shadow-2 bg-zinc-100 rounded-md p-3">
      <div className="flex items-start">
        {comment?.userInfo?.userImage || comment?.user?.avatar ? (
          <>
            <Link href="/community/feed" passHref aria-label="community">
              <div className="w-10 h-10 rounded-full flex items-center justify-center">
                <img src={comment?.userInfo?.userImage || comment?.user?.avatar} alt="" className="rounded-full" />
              </div>
            </Link>
          </>
        ) : (
        ...
        <div className="ml-4 break-all" style={{ width: "calc(100% - 39px)" }}>
          <Link href={`/community/feed`} passHref aria-label="community">
          ...
          <div className="flex justify-between items-center mt-5 ">
            ...
            >
              {isLiked ? (
                <ImageComponent
                  alt="like"
                  src={getStaticUrl("/images/bbc-g3/heart-selected.svg")}
                  width="22.4px"
                  height="22.4px"
                />
              ) : (
                <ImageComponent alt="unlike" src={getStaticUrl("/images/bbc-g3/heart.svg")} width="22.4px" height="22.4px" />
              )}

              <span className={`ml-0.5 text-xs font-normal mr-1 ${isLiked ? "text-red-400" : "text-gray-600"}`}>Like</span>
              <span className={`text-xs font-normal ${isLiked ? "text-red-400" : "text-gray-600"}`}>
                ({comment.likeCount || comment.likes_count || 0})
              </span>
            </div>
            {commentDate ? <p className="text-xs font-normal text-gray-600">{timeSince(commentDate)} ago</p> : null}
          </div>
