...
const PDPWidgetsHomeSync = ({ product, item }: { product: PDPProd; item: any }) => {
  const PDPWidgetGroup: any = (item: any) => {
    switch (item?.customParam) {
      ...
      default:
        console.info("Unknown Custom Param Introduced in PDP Widgets");
        return <div hidden />;
    }
  };
  ...
  return (
    <ErrorBoundary>
      <LazyHydrate whenIdle>{PDPWidgetGroup(item)}</LazyHydrate>
    </ErrorBoundary>
  );
};

export default PDPWidgetsHomeSync;
