import { ALL_ALPHABETS, GENDER, ORIGIN_LIST, RandomAlphabets, REGIONAL_LIST } from "@libConstants/BabyNamesConstants";
import { GBC_ENV } from "@libConstants/GBC_ENV.constant";
import { BabyNamesObject, filterType } from "@typesLib/babyNamesTypes";
...
export const getBabyNamesFilter = (pagePath: any, pageNumber: number, limit: number): filterType => {
...
export const getPageData = (pagePath: any, totalNames: number, pageNumber: number) => {
  const slug1 = pagePath?.[0];
  const slug2 = pagePath?.[1];
  if (slug1?.includes("-meaning")) {
...
export const removeDuplicateBabyName = (list: BabyNamesObject[], matchKey: string, value: string) => {
  if (list?.length) {
    return list?.filter((item: any) => item?.[matchKey] !== value);
  }
};
...
export const getAlphabetRelatedAlphabets = (alphabet: string) => {
  const alphabetsArray = (RandomAlphabets as any)[alphabet.split("starting-with-")[1].toLocaleUpperCase()];
  const generatedLinks = alphabetsArray?.map((item: string) => {
