...
export function getPageSlots(selectedPage:number, noOfSlots:number, maxPages:number) {
  let pageSlot = [];
...
export function getAssociatedSlugForArticles(pathname:string) {
  if (pathname === "/baby") {
    return "new-parents";
  }
...
export function getAssociatedLifestageSEOContent(pathname:string) {
  if (pathname === "/baby") {
    return {
...
export default function decodeEntities(encodedString:string) {
